...
def test_setup_window_exposes_compounds_page(qtbot: QtBot):
    ...
    assert isinstance(window.inventory_widget, InventoryWorkspace)
    assert not hasattr(window.compounds_widget, "edit_inventory_button")
    ...
    assert window.stackWidget.indexOf(window.inventory_widget) >= 0
    assert (
        window.navigationInterface.widget(window.inventory_widget.objectName())
        is not None
    )
