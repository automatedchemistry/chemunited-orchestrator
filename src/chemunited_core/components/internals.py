...
class InternalEdge:
    ...
    Attributes:
        ...
        diameter:            Channel inner diameter in metres
                                (used for resistance calc).
        role:                TRANSPORT (geometry matters) or JUNCTION (lossless).
        resistance_override: Fixed resistance in Pa·s/m³; None =
                             compute from geometry.
        forced_flow_override: Fixed volumetric flow in m³/s; None = flow is
                             solved from resistance as normal. When set, the
                             sim solver treats this edge as an ideal flow
                             source (current source) — its own conductance is
                             excluded from the hydraulic network and the exact
                             value passes through regardless of the pressure
                             field on either side. Mutually exclusive with a
                             closed edge; close() always clears this.
    """
    ...
    forced_flow_override: float | None = None
    ...
    def close(self) -> Self:
        ...
        self.forced_flow_override = None
        return self
