...
class CustomProcess(Process[ProcessConfig]):
    ...
    def build_workflow(self) -> nx.DiGraph:
        ...
        def add_node(
            node_id: str, method: str, label: str, description: str, delay_s: float
        ) -> None:
            graph.add_node(
                node_id,
                **WorkflowNodeSpec(
        ...
        add_node(
            ...
            "handle_low_pressure",
            "Fallback branch",
            "Runs only when the initial check is false",
            2,
        )
        add_node(
            "2", "join_primary_paths", "Join", "Joins only the active predecessors", 1
        )
        ...
        graph.add_edge(
            "IN",
            "0",
            **WorkflowEdgeSpec(condition=True, label="start").model_dump(
                exclude_none=True
            ),
        )
        graph.add_edge(
            "0",
            "1",
            **WorkflowEdgeSpec(condition=False, label="low pressure").model_dump(
                exclude_none=True
            ),
        )
        graph.add_edge(
            "0",
            "3",
            **WorkflowEdgeSpec(condition=True, label="primary").model_dump(
                exclude_none=True
            ),
        )
        graph.add_edge(
            "0",
            "4",
            **WorkflowEdgeSpec(condition=True, label="fan-out").model_dump(
                exclude_none=True
            ),
        )
        graph.add_edge(
            "1",
            "2",
            **WorkflowEdgeSpec(condition=True, label="recover").model_dump(
                exclude_none=True
            ),
        )
        graph.add_edge(
            "3",
            "2",
            **WorkflowEdgeSpec(condition=True, label="join").model_dump(
                exclude_none=True
            ),
        )
        graph.add_edge(
            "4",
            "5",
            **WorkflowEdgeSpec(condition=True, label="report").model_dump(
                exclude_none=True
            ),
        )
        graph.add_edge(
            "4",
            "6",
            **WorkflowEdgeSpec(condition=True, label="measure").model_dump(
                exclude_none=True
            ),
        )
        graph.add_edge(
            "6",
            "7",
            **WorkflowEdgeSpec(condition=True, label="validate").model_dump(
                exclude_none=True
            ),
        )
        graph.add_edge(
            "2",
            "OUT",
            **WorkflowEdgeSpec(condition=True, label="done").model_dump(
                exclude_none=True
            ),
        )
        graph.add_edge(
    ...
    def check_pressure(self, ctx: NodeExecutionContext) -> bool:
        ...
        ctx.runtime.status_message = (
            f"Primary route selected: {self.config.primary_route}"
        )
    ...
    def join_primary_paths(self, ctx: NodeExecutionContext) -> bool:
        self._pause(ctx)
        ctx.runtime.status_message = (
            "Join node ran after only the active predecessors completed."
        )
        return True
    ...
    def prepare_analysis(self, ctx: NodeExecutionContext) -> bool:
        self._pause(ctx)
        ctx.runtime.status_message = (
            f"Preparing analysis for iteration {ctx.iteration}."
        )
        return True
    ...
    def validate_signal(self, ctx: NodeExecutionContext) -> bool:
        ...
        ctx.runtime.status_message = (
            f"Validation passed={passed} for iteration {ctx.iteration}."
        )
        return passed
